...
@property (strong,nonatomic) YXActionSheetView * actionSheetView;
@property (strong,nonatomic) MJPhotoView * longPressMjPhotoView;
@end
...
- (void)createActionSheetView{
    self.actionSheetView = [YXActionSheetView loadFromXib];
    self.actionSheetView.width = [[UIScreen mainScreen] bounds].size.width;
    __weak __typeof(self)weakSelf = self;
    [self.actionSheetView addSheetItemWithItemType:SheetViewItemTypeSave HandleBlock:^{
        NSLog(@"saveing ...");
        UIImage * image = weakSelf.longPressMjPhotoView.photo.image;
        if(image){
            UIImageWriteToSavedPhotosAlbum(image, weakSelf, @selector(image:didFinishSavingWithError:contextInfo:), nil);
        }
    }];
    [self.actionSheetView addSheetItemWithItemType:SheetViewItemTypeReport HandleBlock:^{
...
-(void)photoViewLongPressGeture:(MJPhotoView *)photoView{
    self.longPressMjPhotoView = photoView;
    if(self.actionSheetView.actionSheetViewStatus == SheetViewItemTypeHidden){
        [self.actionSheetView showActionSheetView:YES];
    }
