...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        [_imageView setImageWithURL:_photo.url placeholderImage:_photo.srcImageView.image options:SDWebImageRetryFailed|SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            if (receivedSize > kMinProgress) {
                if([weakSelf.photoLoadingView respondsToSelector:@selector(setProgress:)]){
...
- (void)longPressGestureHandle:(UILongPressGestureRecognizer*)longPressGesture{
    NSLog(@"long press Time is %@",[NSDate date]);
    if([self.photoViewDelegate respondsToSelector:@selector(photoViewLongPressGeture:)]){
        [self.photoViewDelegate photoViewLongPressGeture:self];
    }
}
- (void)dealloc
{
