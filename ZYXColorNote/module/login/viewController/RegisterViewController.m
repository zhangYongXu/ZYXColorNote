...
@property (strong,nonatomic) UserModel * registerUserModel;
@end
...
-(UserModel *)registerUserModel{
    if(nil == _registerUserModel){
        _registerUserModel = [[UserModel alloc] init];
    }
...
- (IBAction)getVerifyCodeBtn:(UIButton*)sender {
    if([sender.titleLabel.text isEqualToString:@"获取验证码"]){
        ...
        [APPDelegate.userViewModel requestGetVerifyCodeWithPhoneNumber:mobile SuccessBlock:^(id object) {
            ...
            NSString * verifyCode = [self randomVerifyCode];
            
            [self.getVerifyCodeBtn setTitle:verifyCode forState:UIControlStateNormal];
...
-(NSString*)randomVerifyCode{
    NSString * digitalStr = @"1234567890";
    NSString * verifyCode = @"";
    
    for(NSInteger i = 0;i<6;i++){
        NSInteger index = arc4random_uniform((int)digitalStr.length-2);
        NSString * d = [digitalStr substringWithRange:NSMakeRange(index, 1)];
        verifyCode = [verifyCode stringByAppendingString:d];
    }
...
- (BOOL)validate{
    ...
    if(![self.verifyCode isEqualToString:self.getVerifyCodeBtn.titleLabel.text]){
...
//    [APPDelegate.userViewModel requestValidateVerifyCode:self.verifyCode SuccessBlock:^(id object) {
        weakSelf.registerUserModel.phone = weakSelf.phone;
        weakSelf.registerUserModel.user_pwd = weakSelf.password;
        [APPDelegate.userViewModel requestUserRegisterWithUserModel:weakSelf.registerUserModel SuccessBlock:^(id object) {
            [APPDelegate.userViewModel requestUserLoginWithUserModel:weakSelf.registerUserModel SuccessBlock:^(id object) {
